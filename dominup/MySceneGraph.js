...
MySceneGraph.prototype.onXMLReady=function(){
	...
	errors = this.parseAnimations(rootElement);
	if(this.criticalErrors(errors))
		return null;
 ...
 * @return set of coordenates of type [x, y, z], null if an error occurred
 */
MySceneGraph.prototype.getControlPoint= function(element, required) {
	var coordenates = this.getXYZ(element, ['x', 'y','z'], required);
...
MySceneGraph.prototype.checkStructure= function(elementType, element, syntax) {
	...
	for(nC=0, nS=0; nC < element.children.length && nS < syntax.length; nC++)
		...
		|| element.children[nC].nodeName == 'ROTATION' 
		|| element.children[nC].nodeName == 'SCALE' 
		|| element.children[nC].nodeName == 'ANIMATIONSEQ')
			continue;
		else
 ...
 */
MySceneGraph.prototype.parseGenericElement= function(elementType, rootElement) {
	...
	for(n=0; n<elems.length; n++){
		...
		for(m=0; m<elmsTemp.length; m++){
			...
			if(this[elementType.toLowerCase() +'s']['#' + elm.id]!= null)
				...
				this[elementType.toLowerCase() +'s']['#' + elm.id]=this.readAnimation(elm);
			else continue;
 ...
 */
MySceneGraph.prototype.readAnimation = function(element) {

	var animation = [];

	animation['span'] = this.reader.getFloat(element, 'span', false);
	if(animation['span']==NaN)
		this.onXMLWarning("Error on <ANIMATION id=" + element.id +">/<span>. Element ignored.");

	animation['type'] = this.reader.getString(element, 'type', false);
	if(animation['type']==null){
		this.onXMLWarning("Error on <ANIMATION id=" + element.id +">/<type>. Element ignored.");
		return null;
	}
	...
	switch(animation['type']){
		case 'linear':
			...
			var controlPoints =  element.getElementsByTagName('CONTROLPOINT');
			for(var n=0; n< controlPoints.length; n++){
				var controlPoint = this.getControlPoint(controlPoints[n], false);

				if(controlPoint!=null)
					animation['controlPoints'].push(controlPoint);
				else this.onXMLWarning("Error on <ANIMATION id=" + element.id +">/<CONTROLPOINT> (L." + (n + 1) +"). Element ignored."); 
			}
		break;
		case 'circular':
				var attributes = ['center', 'radius', 'startang', 'rotang'];
				
				for(var n=0; n< attributes.length; n++){

					if(attributes[n]=='center'){
					
						var arg = this.reader.getString(element, attributes[n], false);
						if(arg==null){
							this.onXMLWarning("Error on <ANIMATION id=" + element.id +">/<" + attributes[n] + ">. Element ignored.");
							continue;
						}

						animation[attributes[n]]=[];
						var values = arg.split(" ");
						for(var i=0; i<values.length; i++)
							animation[attributes[n]].push(parseFloat(values[i]));
					}else{

						var arg = this.reader.getFloat(element, attributes[n], false);
...
MySceneGraph.prototype.readLeaf= function(element) {
	...
	switch(leaf['type']){
		case 'plane':
			this.checkStructure('LEAF', element, ['type', 'parts']);

			leaf['parts'] = this.reader.getInteger(element, 'parts', false);
			if(leaf['parts']==null || isNaN(leaf['parts'])){
				this.onXMLWarning("Error on <LEAF id=" + element.id +">/<parts>. Element ignored.");
				return null;
			}
			break;

		case 'patch':
			leaf['order'] = this.reader.getInteger(element, 'order', false);
			if(leaf['order']==null || isNaN(leaf['order'])){
				this.onXMLWarning("Error on <LEAF id=" + element.id +">/<order>. Element ignored.");
				return null;
			}

			leaf['partsU'] = this.reader.getInteger(element, 'partsU', false);
			if(leaf['partsU']==null || isNaN(leaf['partsU'])){
				this.onXMLWarning("Error on <LEAF id=" + element.id +">/<partsU>. Element ignored.");
				return null;
			}

			leaf['partsV'] = this.reader.getInteger(element, 'partsV', false);
			if(leaf['partsV']==null || isNaN(leaf['partsV'])){
				this.onXMLWarning("Error on <LEAF id=" + element.id +">/<partsV>. Element ignored.");
				return null;
			}
			...
			for(var n=0; n< controlPoints.length; n++){
				...
				if(controlPoint!=null)
					leaf['controlPoints'].push(controlPoint);
				else this.onXMLWarning("Error on <LEAF id=" + element.id +">/<CONTROLPOINT> (L." + (n + 1) +"). Element ignored."); 
			}

			if(leaf['controlPoints'].length!= Math.pow(leaf['order']+1, 2)){
				this.onXMLWarning("Error on <LEAF id=" + element.id +">/<CONTROLPOINT> (L." + (n + 1) +"). Element ignored."); 
				return null;
			}	
			break;
		...
		case 'terrain':
			this.checkStructure('LEAF', element, ['type', 'texture', 'heightMap']);

			leaf['texture'] =  this.reader.getString(element, 'texture', false);
			if(leaf['texture'] ==null){
				this.onXMLWarning("Error on <LEAF id=" + element.id +">/<texture>. Element ignored.");
				return null;
			} 

			leaf['texture'] = this.path + leaf['texture'];

			leaf['heightMap'] = this.reader.getString(element, 'heightMap', false);
			if(leaf['heightMap']==null){
				this.onXMLWarning("Error on <LEAF id=" + element.id +">/<'heightMap'>. Element ignored.");
				return null;
			}

			leaf['heightMap'] = this.path + leaf['heightMap'];
			break;
...
MySceneGraph.prototype.readNode= function(element) {
	...
	// read animations
	node['animations'] = [];
	var animations =  element.getElementsByTagName('ANIMATIONSEQ');
	
	var n, m;
	for(n=0; n<animations.length; n++){
		var animationsTemp = animations[n].getElementsByTagName('ANIMATION');
		for(m=0; m<animationsTemp.length; m++)
			node['animations'].push('#' + animationsTemp[m].id);
	}
	...
	for(n=0; n<element.children.length; n++){
		...
		if(transformation['args']!=null)
			...
			this.onXMLWarning("Error on <NODE id=" + element.id +"><"+ element.children[n].nodeName + "> (L. "+n+"). Element ignored.");
	}
...
MySceneGraph.prototype.processGraph = function(elementId) {
	...
	// check animations
	var n;
	for(n=0; n< element['animations'].length; n++)
		if(this.animations[element['animations'][n]] == null){
			this.onXMLWarning("Error on <NODE> with id = " + elementId + " - No <ANIMATION> with id=" + element['animations'][n] + ".");
			element['animations'].splice(n, 1);
			n--;
		}

	if(element['animations'].length>0)
		element['activeAnimation']=0;
	else element['activeAnimation']=undefined;
